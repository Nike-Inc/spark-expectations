...
def test_send_notification_success(_mock_context):
    ...
    with patch("spark_expectations.notifications.plugins.email.smtplib.SMTP") as mock_smtp, patch(
...
def test_send_notification_exception(_mock_context):
    ...
    with patch("spark_expectations.notifications.plugins.email.smtplib.SMTP") as mock_smtp, pytest.raises(
        SparkExpectationsEmailException
    ):
