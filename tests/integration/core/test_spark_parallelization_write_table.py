...
def filter_and_group_positive_numbers_to_table(df, column_name, table_name):
    """
...
def test_write_to_table(spark_session):
    spark = spark_session
    data = [(1,), (2,), (3,), (4,), (5,)]
    df = spark.createDataFrame(data, ["value"])
    table_name = f"test_table_{uuid.uuid4().hex}"
    filter_and_group_positive_numbers_to_table(df, "value", table_name)
    ...
    # Optionally, drop the table after test
...
def test_small_dataset(spark_session):
    ...
    data = [(-2,), (0,), (1,), (2,), (3,), (4,)]
    df = spark.createDataFrame(data, ["value"])
    result_df = filter_and_group_positive_numbers_to_table(df, "value")
    result = {row.mod_group: row.sum_value for row in result_df.collect()}
    expected = {
        0: 3,
        1: 5,
