...
def spark(tmp_path_factory, worker_id):
    ...
    warehouse_dir = tmp_path_factory.mktemp(f"warehouse_{worker_id}")
    derby_dir = tmp_path_factory.mktemp(f"derby_{worker_id}")
    builder = (
...
@pytest.fixture(name="_fixture_local_kafka_topic",scope="function", autouse=True)
def fixture_setup_local_kafka_topic():
    ...
    if os.getenv("UNIT_TESTING_ENV") != "spark_expectations_unit_testing_on_github_actions":
        ...
        # os.system(f"sh {current_dir}/../../../spark_expectations/examples/docker_scripts/docker_kafka_start_script_parallel.sh")
...
def fixture_create_database(spark, tmp_path_factory, worker_id):
    # try: 
    ...
    warehouse_dir = spark.conf.get("spark.sql.warehouse.dir")  # Retrieve from conf
    print(f"~*_~*_~*_~*_~*_~*_ WAREHOUSE DIR: {warehouse_dir} ~*_~*_~*_~*_~*_~*_")

    if warehouse_dir.startswith("file:"):
        warehouse_dir = warehouse_dir.replace("file:", "", 1)
 
    # drop and create dq_spark if exists
    os.system(f"rm -rf {warehouse_dir}/dq_spark.db")
    spark.sql("create database if not exists dq_spark")
    spark.sql("use dq_spark")
...
# def fixture_create_stats_table(spark):
...
def test_with_expectations(
    ...
    se._context.set_se_streaming_stats_topic_name(f"test_topic_{worker_id}")
    ...
    b = stats_table.selectExpr("to_json(struct(*)) AS value").collect()
