...
@pytest.mark.parametrize(
    ...
    [
        ...
        (  # test case 4
            ...
            True,  # write to table
            spark.createDataFrame(
                [{"col1": 1, "col2": "a"}, {"col1": 2, "col2": "b"}, {"col1": 3, "col2": "c"}]  # expected_output
            ),
            None,  # expected agg dq result
            # status at different stages for given input
            {
...
@patch("spark_expectations.utils.regulate_flow.SparkExpectationsNotify", autospec=True, spec_set=True)
def test_execute_dq_process(
    _mock_notify,
    df,
    expectations,
    rule_type,
    ...
    # assert expectations result
        ...
        (df, _agg_dq_res, _error_count, _status) = func_process(
            df,
            rule_type,
            row_dq_flag,
            source_agg_dq_flag,
        ...
        if rule_type == "row_dq":
            ...
            assert _agg_dq_res == agg_or_query_dq_res
            assert _status == (
                status.get("source_query_dq_status")
                if source_query_dq_flag is True
                else status.get("final_query_dq_status")
            )
...
def test_execute_dq_process_exception(
    df,
    expectations,
    rule_type,
    row_dq_flag,
    ...
    with pytest.raises(SparkExpectationsMiscException, match=r"error occurred while executing func_process .*"):
