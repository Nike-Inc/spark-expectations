...
@patch("spark_expectations.notifications.plugins.pagerduty.SparkExpectationsContext", autospec=True, spec_set=True)
def test_get_pd_integration_key(_mock_context):
    pagerduty_handler = SparkExpectationsPagerDutyPluginImpl()
    _mock_context.get_pagerduty_integration_key = "test_integration_key"
    ...
    with patch("spark_expectations.notifications.plugins.pagerduty.SparkExpectationsPagerDutyPluginImpl._get_pd_integration_key", return_value="test_integration_key") as mock_retrieve_integration_key:
        pagerduty_handler._get_pd_integration_key(_mock_context)
        mock_retrieve_integration_key.assert_called_once()


@patch("spark_expectations.notifications.plugins.pagerduty.SparkExpectationsContext", autospec=True, spec_set=True)
def test_get_pd_integration_key_missing_key(_mock_context):
    pagerduty_handler = SparkExpectationsPagerDutyPluginImpl()
    ...
    with patch("spark_expectations.notifications.plugins.pagerduty.SparkExpectationsPagerDutyPluginImpl._get_pd_integration_key", return_value=None, side_effect=KeyError("KeyError: PagerDuty integration key is missing.")) as mock_retrieve_integration_key:
        with pytest.raises(KeyError, match="KeyError: PagerDuty integration key is missing.") as exception_info:
            pagerduty_handler._get_pd_integration_key(_mock_context)
        assert "KeyError: PagerDuty integration key is missing." in str(exception_info.value)
