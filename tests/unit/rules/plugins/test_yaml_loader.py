...
def test_handles_yaml_format_explicit(yaml_loader, dq_env_yaml_file):
    df = yaml_loader.load_rules(path=dq_env_yaml_file, format="yaml", options={"dq_env": "DEV"})
    assert df is not None
    assert df.count() == 3
    assert set(df.columns) == set(RULES_SCHEMA_COLUMNS)


def test_handles_auto_yaml_extension(yaml_loader, dq_env_yaml_file):
    df = yaml_loader.load_rules(path=dq_env_yaml_file, format="auto", options={"dq_env": "DEV"})
    assert df is not None
    assert df.count() == 3


def test_handles_auto_yml_extension(yaml_loader, dq_env_yaml_file, tmp_path):
    yml_path = tmp_path / "rules.yml"
    import shutil
    shutil.copy(dq_env_yaml_file, str(yml_path))
    df = yaml_loader.load_rules(path=str(yml_path), format="auto", options={"dq_env": "DEV"})
    assert df is not None
    assert df.count() == 3


def test_dq_env_values(yaml_loader, dq_env_yaml_file):
    df = yaml_loader.load_rules(path=dq_env_yaml_file, format="yaml", options={"dq_env": "DEV"})
...
def test_dq_env_selects_prod(yaml_loader, dq_env_yaml_file):
    df = yaml_loader.load_rules(path=dq_env_yaml_file, format="yaml", options={"dq_env": "PROD"})
    rows = [r.asDict() for r in df.collect()]
...
def test_sample_rules_yaml_env_table_name(yaml_loader):
    """Verify that table_name changes per environment."""
    df_dev = yaml_loader.load_rules(path=SAMPLE_RULES_YAML, format="yaml", options={"dq_env": "DEV"})
    df_qa = yaml_loader.load_rules(path=SAMPLE_RULES_YAML, format="yaml", options={"dq_env": "QA"})
...
def test_no_active_spark_session_raises(yaml_loader, dq_env_yaml_file):
