    ...
    def dq_obs_report_data_insert(self) -> tuple[bool, DataFrame]:
        try:
            ...
            only_querydq_src_df = only_querydq_src_base_df.withColumn('extracted_total_records_data', split(
                ...
                .withColumn('total_records_dict_split',
                            split(regexp_replace(col('total_records_dict'), '[}{]', ''), ',')) \
                .withColumn('total_records', expr("element_at(total_records_dict_split, -1)")) \
                .withColumn('column_name', when(size(col('total_records_dict_split')) > 1, concat_ws(",", expr(
                "slice(total_records_dict_split, 1, size(total_records_dict_split)-1)"))).otherwise(col('column_name')))
            ...
            only_querydq_tgt_df = only_querydq_tgt_base_df.withColumn('extracted_valid_records_data', split(
                ...
                .withColumn('total_valid_dict_split', split(regexp_replace(col('valid_records_dict'), '[}{]', ''), ',')) \
                .withColumn('valid_records', expr("element_at(total_valid_dict_split, -1)")) \
                .withColumn('column_name', when(size(col('total_valid_dict_split')) > 1, concat_ws(",", expr(
                "slice(total_valid_dict_split, 1, size(total_valid_dict_split)-1)"))).otherwise(col('column_name')))
            ...
            only_querydq_final_after_join_df = only_querydq_final_after_join_df \
                ...
                .withColumn('success_percentage',
                            when((col('total_records_only_nbr') == '') & (col('valid_records_only_nbr').isNull()),
                                 lit(100))
                            .when((col('total_records_only_nbr') == '') & (col('valid_records_only_nbr') == ''),
                                  lit(100))
                            .when((col('total_records_only_nbr') != '') & (col('valid_records_only_nbr').isNull()),
                                  lit(0))
                            .otherwise(coalesce(
